...
If you don't know the keynames, use brackets! You can use *variables* to identify unknown or weird keynames using the syntax `object[property_name]`, where `property_name` can be represented by a string or Symbol. Because JavaScript relies upon type coercion, these values will be coerced into strings.
...
```
`${jediOne.name} has a pair of lightsabers with 
${jediOne.lightsabers[2].blade} blades.`
```
