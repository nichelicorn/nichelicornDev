---
title : "Revisiting Concepts : Dot and Bracket Notation"
---
...
# Revisiting Concepts : Dot and Bracket Notation
Today's review topic was dot and bracket notation. What are they for? Why would I use one or the other? Is it even important to know the differences? How do they work?

Here is what I learned about property accessors.
...
The `property` is the key you are attempting to access within an object (and get its values). Property accessors give us the ability to obtain these values. Neat!

### How to choose?

Start by asking yourself a question : "Do I know the keynames I want to access?"
...
### Bracket notation
Bracket notation provides more flexibility when it comes to accessing values dynamically. 

Interestingly, when using bracket notation, the key name does not need to be known! JavaScript has the power to search an object for a matching property, and return that value if a match is found. This gives us some flexibility with how we interact with objects.

Let's see if we can discover the Jedi's homeworld using this power (and of course, pretend you haven't already examined this property).

```
const mysteryPlanet = "homeworld";

console.log(jediOne[mysteryPlanet]);
```
...
You should now have an array of lightsabers that Ahsoka can use! And we can use their values by using some fancy notation.
...
```
console.log(`${jediOne.name} has a pair of lightsabers with ${jediOne["lightsabers"][2].blade} blades.`)
```

Or maybe something else? Did you get the result you wanted! If you did, yay!!! 🎉 If you didn't, that's okay -- try again!

<sub>
Thank you for reading my post! Check out these resources if you want more info about dot / bracket notation.
</sub>
<sub>
  <ul>
